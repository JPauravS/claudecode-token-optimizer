...
import { getWolfDir, ensureWolfDir, readJSON, readMarkdown, readStdin, getWorkspaceWolfDir /* LOCAL MOD: getWorkspaceWolfDir */ } from "./shared.js";
async function main() {
    ensureWolfDir();
...
function checkCerebrum(wolfDir, content) {
    const cerebrumContent = readMarkdown(path.join(getWorkspaceWolfDir(), "cerebrum.md") /* LOCAL MOD (claude-stack): pre-write cerebrum → workspace */);
...
function checkBugLog(wolfDir, filePath, oldStr, newStr, content) {
    const bugLogPath = path.join(getWorkspaceWolfDir(), "buglog.json") /* LOCAL MOD (claude-stack): pre-write buglog → workspace */;
