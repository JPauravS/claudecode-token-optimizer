...
import { getWolfDir, ensureWolfDir, readJSON, writeJSON, parseAnatomy, serializeAnatomy, extractDescription, estimateTokens, appendMarkdown, timeShort, readStdin, normalizePath, getWorkspaceWolfDir /* LOCAL MOD: getWorkspaceWolfDir */ } from "./shared.js";
async function main() {
    ensureWolfDir();
...
function autoDetectBugFix(wolfDir, absolutePath, projectRoot, oldStr, newStr) {
    const bugLogPath = path.join(getWorkspaceWolfDir(), "buglog.json") /* LOCAL MOD (claude-stack): buglog → workspace */;
    ...
    if (recentDupe) {
        ...
        if (!recentDupe.project_origin)
            recentDupe.project_origin = process.env.CLAUDE_PROJECT_DIR || projectRoot; /* LOCAL MOD (claude-stack): buglog project_origin */
