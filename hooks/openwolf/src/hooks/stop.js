...
import { getWolfDir, ensureWolfDir, readJSON, writeJSON, appendMarkdown, timeShort, getWorkspaceWolfDir /* LOCAL MOD: getWorkspaceWolfDir */ } from "./shared.js";
async function main() {
    ensureWolfDir();
    ...
    session.stop_count++;
    const isFirstStop = session.stop_count === 1; /* LOCAL MOD (claude-stack): isFirstStop gate */
    ...
    if (isFirstStop) { /* LOCAL MOD (claude-stack): isFirstStop gate */
    ...
    /* LOCAL MOD (claude-stack): ledger dedupe + delta */
    const _priorIdx = ledger.sessions.findIndex((s) => s && s.id === sessionEntry.id);
    const _priorSnap = (_priorIdx >= 0 && ledger.sessions[_priorIdx] && ledger.sessions[_priorIdx]._snapshot)
        ? ledger.sessions[_priorIdx]._snapshot
    ...
    if (_priorIdx >= 0)
        ledger.sessions[_priorIdx] = sessionEntry;
    else
        ledger.sessions.push(sessionEntry);
    ledger.lifetime.total_reads += readCount - _priorSnap.reads; /* LOCAL MOD (claude-stack): ledger dedupe + delta */
    ...
    const savedFromRepeats = Object.values(session.files_read)
        .filter((r) => r.count > 1)
        .reduce((sum, r) => sum + r.tokens * (r.count - 1), 0);
    ledger.lifetime.estimated_savings_vs_bare_cli += (savedFromAnatomy + savedFromRepeats) - _priorSnap.estimated_savings; /* LOCAL MOD (claude-stack): ledger dedupe + delta */
    writeJSON(ledgerPath, ledger);
    // Write a session summary line to memory.md if there was meaningful activity
    if (isFirstStop && writeCount > 0) { /* LOCAL MOD (claude-stack): isFirstStop gate */
        try {
...
function checkCerebrumFreshness(wolfDir, session) {
    const cerebrumPath = path.join(getWorkspaceWolfDir(), "cerebrum.md") /* LOCAL MOD (claude-stack): cerebrum → workspace */;
    try {
