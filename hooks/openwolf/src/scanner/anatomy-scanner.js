...
function estimateTokens(text, filePath) {
...
function shouldExclude(relPath, excludePatterns) {
    ...
    if (ALWAYS_EXCLUDE_FILES.has(basename))
        return true;
...
function walkDir(dir, rootDir, excludePatterns, maxFiles, entries) {
    let totalFiles = 0;
 ...
 */
export function buildAnatomy(wolfDir, projectRoot) {
    ...
    walkDir(projectRoot, projectRoot, config.openwolf.anatomy.exclude_patterns, config.openwolf.anatomy.max_files, entries);
    let fileCount = 0;
    for (const [, list] of entries)
...
export function scanProject(wolfDir, projectRoot) {
...
export function updateAnatomyEntry(wolfDir, filePath, projectRoot, action) {
    const anatomyPath = path.join(wolfDir, "anatomy.md");
    let content;
    try {
    ...
    if (action === "delete") {
        ...
        const entry = { file: fileName, description: desc, tokens };
